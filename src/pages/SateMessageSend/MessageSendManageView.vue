...
export default {
  ...
  props: ["taskOption"],
  mounted() {
    this.editTask(this.taskOption.task);
  },
  methods: {
    // 返回
    taskPreClick(q) {
    ...
    // 编辑
    editTask(q) {
      INCALLAPI.ecSms_selectById({ id: q.taskId }).then(res => {
        console.log(res);
        console.log(q);
        if (res.status == 200 && res.data.errCode == 0) {
          this.ruleForm.taskDescribe.taskName = q.taskName;
          this.ruleForm.taskDescribe.taskContent = res.data.ecsms.content;
          this.taskPeo.peoSureL = [{name:q.taskReceivePeo}];
        }
      });
    }
  }
