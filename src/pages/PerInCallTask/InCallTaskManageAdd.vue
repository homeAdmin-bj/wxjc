            ...
            <div v-if="taskCycle == 2 && monthTime_timeTpye == 1">
              ...
              <div class="label_select label_select2">
                <el-select v-model="taskTimeSet.monthTime.timeE"
                  placeholder="请选择">
                  <el-option v-for="item in taskTimeDef.timeTpyeLE"
                    :key="item.id"
                    :label="item.value"
                    :value="item.id">
                  </el-option>
...
export default {
  data() {
    ...
    var valtaskName = (rule, value, callback) => {
      ...
      this.ruleForm.outCallTactics.intervaltime = value;
      callback();
    };
    var outCallTimes = (rule, value, callback) => {
      ...
      value = value.replace(/[^\d]/g, "");
      this.ruleForm.outCallTactics.outCallTimes = value;
      callback();
    };
    var outCallDuration = (rule, value, callback) => {
      ...
      value = value.replace(/[^\d]/g, "");
      this.ruleForm.outCallTactics.outCallDuration = value;
      callback();
    };
    return {
      treeLoading: false,
  ...
  methods: {
        ...
        if (this.taskOption.taskD == undefined) {
          ...
          if (this.taskOption.addType == 1) {
            ...
            this.$refs["ruleForm"].validate(valid => {
              if (valid) {
                ...
                INCALLAPI.ecTaskIn_saveGeneralEcTaskIn(options).then(res => {
                  if (res.status == 200 && res.data.errCode == 0) {
                    this.$message({
                      type: "success",
                      message: "添加成功!"
            ...
            this.$refs["ruleForm"].validate(valid => {
              if (valid) {
                ...
                INCALLAPI.ecTaskIn_saveCycleEcTaskIn(options).then(res => {
                  if (res.status == 200 && res.data.errCode == 0) {
                    this.$message({
                      type: "success",
                      message: "添加成功!"
          ...
          this.$refs["ruleForm"].validate(valid => {
            if (valid) {
              ...
              INCALLAPI.ecTaskIn_updateByEcTaskInfo(options).then(res => {
                if (res.status == 200 && res.data.errCode == 0) {
                  this.$message({
                    type: "success",
                    message: "修改成功!"
    ...
    editTask(id) {
      ...
      INCALLAPI.ecTaskIn_selectById(options).then(res => {
        if (res.status == 200 && res.data.errCode == 0) {
          if (res.data.ecTaskIn != undefined) {
            this.ruleForm.taskDescribe.taskName = res.data.ecTaskIn.name;
            this.ruleForm.taskDescribe.taskTypeId = res.data.ecTaskIn.type;
            this.ruleForm.taskTimeSet.BEtime.day = [
              new Date(res.data.ecTaskIn.startTime.split(" ")[0]).getTime(),
              new Date(res.data.ecTaskIn.endTime.split(" ")[0]).getTime()
            ];
            INCALLAPI.ecTaskIn_findByEcTaskInfo({
              content: res.data.ecTaskIn.content,
              depElect: res.data.ecTaskIn.depElect
            }).then(res => {
              if (res.status == 200 && res.data.errCode == 0) {
                console.log(res);
                var contentName = [];
            ...
            this.taskPeo.peoIdSureL = res.data.ecTaskIn.content.split(",");
            this.taskPeo.selectedPeoIdL = res.data.ecTaskIn.content.split(",");
          }
        }
      });
    }
  ...
  watch: {
    ...
    "taskTimeSet.monthTime.timeB": function(q) {
      var num = 31;
      var numList = [];
      for (let index = q; index < num + 1; index++) {
        var item = {};
        item.value = index;
        item.id = index;
        numList.push(item);
      }
      this.taskTimeDef.timeTpyeLE = numList;
      this.taskTimeSet.monthTime.timeE = q;
    }
  }
};
</script>
