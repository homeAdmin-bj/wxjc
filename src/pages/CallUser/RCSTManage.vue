<template>
  <div style="width:100%;height:100%;position:relative">
    <div id="RCSTManage" v-show="showString=='Manage'">
      <div class='title'>
        拨测用户管理 > 卫星终端
      </div>
      <div class='leftbar'>
            ...
            >
              <template slot-scope="scope">
                <i v-if="!isCommonR"
                  ...
                  title="编辑"
                  @click="taskUpdate(scope.row,isCommonR)"
                ></i>
                &nbsp;
                <i v-if="!isCommonR"
                  ...
                  style="font-size:20px;line-height: 39px;"
                  title="查看"
                  @click="taskView(scope.row,isCommonR)"
                ></i>
                &nbsp;
                <i
                  ...
                  style="font-size:20px;line-height: 39px;"
                  title="删除"
                  @click="taskDel(scope.row,isCommonR)"
                ></i>
              </template>
            </el-table-column>
          </el-table>
    ...
    <rcst-add @closeManageAdd='handleManageAdd' :info='curInfo' v-show="showString=='ManageAdd'"></rcst-add>
    <rcst-view @closeManageView='handleManageAdd' :info='curInfo' v-show="showString=='ManageView'"></rcst-view>
  </div>
...
import commonBox from "./commonboxr";
import pageBody from "./../../components/page_temp";
import http from "./../../unit/httpServer";
import rcstAdd from './RCSTManageAdd'
import rcstView from './RCSTManageView'
import groupAdd from './RCSTManageAddCommon'
export default {
  data() {
    return {
      groupId:null,
  ...
  computed: {
    isCommonR(){
      return this.$store.getters.getIsCommonR
    }
  },
  mounted() {
    // var vm = this;
  ...
  methods: {
    ...
    manageAdd(){
      console.log('add');
      console.log(this.isCommonR+"this.isCommonR")
      if(this.isCommonR){
    ...
    taskDel(dt,isCommonR) {
      console.log(isCommonR);
      console.log(dt);
      if(isCommonR){
