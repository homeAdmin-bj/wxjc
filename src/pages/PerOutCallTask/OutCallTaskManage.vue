        ...
        <div class="page_box">
          <div class="page_box_requirement">
            <div style="padding-bottom:20px;">
              <div class="requirement">
                ...
                <el-input style="boder-right:none;"
                  v-model="requirement.outTaskName">
                </el-input>
              </div>
              <div class="requirement">
                <label>任务状态：</label>
                <el-select v-model="requirement.outTaskState"
                  placeholder="请选择">
                  <el-option v-for="item in requirement.outTaskStateList"
                    :key="item.id"
                    :label="item.label"
                    :value="item.id">
                  </el-option>
            ...
            <div>
              <div class="requirement">
                <label>任务类型：</label>
                <el-select v-model="requirement.outTaskType"
                  placeholder="请选择">
                  <el-option v-for="item in requirement.outTaskTypeList"
                    :key="item.id"
                    :label="item.label"
                    :value="item.id">
                  </el-option>
              ...
              <div class="requirement">
                ...
                <el-date-picker v-model="requirement.outTaskTime"
                  type="daterange"
                  :editable="false"
                  :clearable="false"
                ...
                <el-table-column fixed="right"
                  ...
                  <template slot-scope="scope">
                    ...
                    <span @click="taskSetOpen(scope.row)"
                      ...
                      style="font-size:14px;line-height: 39px;color: #0978bc;cursor: pointer;">{{scope.row.taskState1 == "taskState3"? "启用":"停用"}}</span>
...
import ManageAdd from "./OutCallTaskManageAdd";
import ManageView from "./../PerOutCallRecord/OutCallRecordManageView";
// api
...
export default {
  data() {
    ...
    return {
      ...
      requirement: {
        ...
        outTaskState: null,
        outTaskType: null,
        outTaskStateList: null,
        outTaskTypeList: null,
        outTaskTime: null
      },
      tableBodyList: null,
      tableHeadList: null
  ...
  mounted() {
    ...
    this.requirement.outTaskTime = [
      this.getFormatDate(new Date(pastResult)),
      this.getFormatDate(new Date())
    ];
    console.log(this.requirement.outTaskTime);
  ...
  methods: {
    ...
    // 任务状态
    setTaskState() {
      this.requirement.outTaskStateList = [
        {
          label: "全部",
          id: null
        },
      ...
      this.requirement.outTaskTypeList = [
        {
          label: "全部",
          id: null
        },
    ...
    getTableBodyData() {
      const options = {
        netType: 1,
        startTime:
          this.requirement.outTaskTime != null
            ? this.requirement.outTaskTime[0]
            : null,
        endTime:
          this.requirement.outTaskTime != null
            ? this.requirement.outTaskTime[1]
            : null,
        name: this.requirement.outTaskName,
        status: this.requirement.outTaskState,
        type: this.requirement.outTaskType,
        pageNo: this.pageInfo.currentPage,
        pageSize: this.pageInfo.pageSize
      };
      SOCTAPI.ecTaskOut_showPage(options).then(res => {
        console.log(res);
        const taskList = [];
    ...
    // 查询重置
    taskQueryReset() {
      this.requirement.outTaskType = null;
      this.requirement.outTaskName = null;
      this.requirement.outTaskState = null;
      const curDate = new Date().getTime();
      const halfYear = (365 / 2) * 24 * 3600 * 1000;
      const pastResult = curDate - halfYear; // 半年前的时间（毫秒单位）
      this.requirement.outTaskTime = [
      ...
      if (true) {
        this.$confirm(
          ...
          .then(() => {
            ...
            SOCTAPI.ecTaskOut_cutEcTaskOutEnable(options).then(res => {
              if (res.status == 200 && res.data.errCode == 0) {
              }
              console.log(res);
              this.$message({
    ...
    taskDel(opt) {
      ...
      if (true) {
        this.$confirm("删除后不可恢复,确认删除当前任务?", "提示", {
          ...
          .then(() => {
            SOCTAPI.ecTaskOut_deleteByIds(options).then(res => {
              if (res.status == 200 && res.data.errCode == 0) {
                this.$message({
                  type: "success",
                  message: "操作成功!"
